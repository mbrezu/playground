...
(ns
  ...
  apocryphal.clojure.contrib.seq-utils)
...
;; Load the LowLevelApocryphal assembly from the same directory as the current file.
(let [current-file (-> (resolve 'positions) meta :file)
      current-dir (System.IO.Path/GetDirectoryName current-file)
      assembly-file (System.IO.Path/Combine current-dir "LowLevelApocryphal.dll")]
  (if (not (System.IO.File/Exists assembly-file))
    (throw (Exception.
            "LowLevelApocryphal.dll should be placed in the same directory as seq_utils.clj"))
    (System.Reflection.Assembly/LoadFile assembly-file)))

(defn shuffle
  "Return a random permutation of coll"
  [coll]
  (let [l (System.Collections.ArrayList. coll)]
    (LowLevelApocryphal.ListExtensions/Shuffle l)
    (seq l))) 
...
(ns 
  ...
  apocryphal.clojure.contrib.str-utils
...
(defn re-split
  ...
  ([#^Regex pattern string limit]
     (let [result (re-split pattern string)
           first-n (take limit result)
           last-one (apply str (drop limit result))]
       (conj (vec first-n) last-one))))
...
(defn re-partition
  ...
  (let [m (re-matcher re string)]
    ((fn step [prevend]
       (lazy-seq
        (if (.find m)
          (cons (subs string prevend (.start m))
                (cons (re-groups m)
                      (step (+ (.start m) (count (.group m))))))
          (when (< prevend (.Length string))
            (list (subs string prevend (.Length string)))))))
     0)))
...
(defn re-gsub
  ...
  If (ifn? replacement) is true, the replacement is called with the
  match.
  "
...
(defn re-sub
  ...
  (if (ifn? replacement)
    (let [m (re-matcher regex string)]
      (if (.find m)
        (str (subs string 0 (.start m))
             (replacement (re-groups m))
             (subs string
                   (+ (.start m)
                      (.Length #^String (.group m)))
                   (.Length string)))
        string))
